 ...
 */
void
stg_MD5Init(struct StgMD5Context *ctx)
{
 ...
 */
void
stg_MD5Update(struct StgMD5Context *ctx, const byte const *buf, unsigned int len)
{
	word32 t;
	...
	byteSwap(ctx->in, 16);
        stg_MD5Transform(ctx->buf, ctx->in);
	buf += (unsigned)t;
	len -= (unsigned)t;
 ...
 */
void
stg_MD5Final(byte digest[16], struct StgMD5Context *ctx)
{
...
void
stg_MD5Transform(word32 buf[4], word32 const in[16])
{
